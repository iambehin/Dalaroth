...
if(!function_exists("get_weapon_damage"))
...
function levelup()
{
	...
	while($_SESSION["userinfo"]["exp"] > $_exptable[$_SESSION["userinfo"]["level"]])
	{
		...
		$gainedhp = mt_rand($_SESSION["userinfo"]["maxhp"]*.1,$_SESSION["userinfo"]["maxhp"]*.2);
		$gainedmp = mt_rand($_SESSION["userinfo"]["maxmp"]*.1,$_SESSION["userinfo"]["maxmp"]*.2);
		$gainedcredits = mt_rand(2,5);
		echo "<span class=warning>You have leveled up!</span>\n<br/>";
		echo "You gain $gainedhp Hit Points.\n<br/>";
		echo "You gain $gainedmp Mana Points.\n<br/>";
		...
		$_SESSION["userinfo"]["maxmp"] +=$gainedmp;
		/*the following is a heal: */
		$_SESSION["userinfo"]["hp"] = $_SESSION["userinfo"]["maxhp"];
		$_SESSION["userinfo"]["mp"] = $_SESSION["userinfo"]["maxmp"];
		update_db("level");
		update_db("maxhp");
...
//This is how we do the acts that only require one query:
   ...
   "credits"=>"credits","ahlon_quest"=>"ahlon_quest","combat"=>"combat","font-family"=>"font-family","vahth_mayor"=>"vahth_mayor","vahth_farm"=>"vahth_farm"
	);
